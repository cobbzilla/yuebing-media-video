...
export const VideoDashOperation = {
    name: "dash",
    command: ffmpegCommand(),
    minFileSize: 128,
};
export const dash = (logger, infile, profile, outDir, sourcePath, conn, analysisResults) => __awaiter(void 0, void 0, void 0, function* () {
    if (!profile.operationConfigObject)
        throw new Error(`dash: profile.operationConfigObject not defined`);
    ...
    if (mediainfoJobs.length !== 1) {
        throw new Error(`dash: expected exactly 1 mediainfo analysisResult for PROFILE=${profile.name} COUNT=${mediainfoJobs.length}`);
    }
    ...
    for (let i = 0; i < profile.subProfileObjects.length; i++) {
        ...
        if (i === profile.subProfileObjects.length - 1) {
            ...
            const subConfig = subProfile.operationConfigObject;
            ...
            if (profileBitrate && profileBitrate <= sourceBitrate) {
                ...
                if (profileWidth && profileWidth <= sourceWidth) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(`dash: skipping SUB_PROFILE=${subProfile.name} because source bitrate/width is less than profile`);
                    }
                }
            }
        }
