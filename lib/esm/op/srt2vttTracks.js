...
export const srt2vttTextTracks = (infile, profile, outDir, sourcePath, conn) => __awaiter(void 0, void 0, void 0, function* () {
    if (profile.ext !== "vtt") {
    ...
    for (let i = 0; i < dirFiles.length; i++) {
        ...
        if (srtMatch && srtMatch[0] === filename) {
            ...
            if (!destOutfileBase.match(textTrackRegex)) {
                console.error(`srt2vttTracks: invalid destOutfileBase (${destOutfileBase}) did not match textTrackRegex=${textTrackRegex}`);
                continue;
            }
